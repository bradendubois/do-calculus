...
def do_calculus_options(graph: Graph, y, x, w):
...
def old_options(graph, rule_1_valid_z, rule_2_valid_z, rule_3_valid_z, y, x, w, current_query):
    ...
    for r1_set in rule_1_valid_z:
        ...
        result_str = query_str(*result)
        y_result, x_result, w_result = result
        assert rule_1_applicable(graph, y_result, x_result, r1_set, w_result), "Rule 1 not found to be reversible!"
        query_message += ", Making: " + current_query + " -> " + result_str
        options.append([CallableItemWrapper(y_result, x_result, w_result), query_message])
    ...
    for r2_set in rule_2_valid_z:
        ...
        result_str = query_str(*result)
        y_result, x_result, w_result = result
        assert rule_2_applicable(graph, y_result, x_result, r2_set, w_result), "Rule 2 not found to be reversible!"
        query_message += ", Making: " + current_query + " -> " + result_str
        options.append([CallableItemWrapper(y_result, x_result, w_result), query_message])
    ...
    for r3_set in rule_3_valid_z:
        ...
        result_str = query_str(*result)
        y_result, x_result, w_result = result
        assert rule_3_applicable(graph, y_result, x_result, r3_set, w_result), "Rule 3 not found to be reversible!"
        query_message += ", Making: " + current_query + " -> " + result_str
        options.append([CallableItemWrapper(y_result, x_result, w_result), query_message])
