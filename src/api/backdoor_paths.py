...
def api_backdoor_paths(bc: BackdoorController, src: set, dst: set, dcf: set) -> list:
    ...
    dst: A set of string vertices that are in the given Backdoor Controller, which will be reached by
        vertices in src.
    dcf: A set of string vertices in the given Backdoor Controller, which will behave as a deconfounding
        set to block paths from src to dst.

    Returns
    -------
    Returns all backdoor paths connecting any vertex in src to a vertex in dst, by which each
    path is represented as a list containing each vertex (as a string) from the source vertex
    to the destination vertex, with dcf acting as a deconfounding set.
    """
    # TODO A method in Backdoor Controller that can return all paths immediately
    paths = []
